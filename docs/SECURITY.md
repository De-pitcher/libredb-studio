...
**3.2.** The process stdout log channel (`libredb.audit.v1`) is the primary stream for server-generated
audit events, including boundary denials emitted by `src/proxy.ts` (such as cross-origin rejections
returning 403 and non-admin `/admin` page redirects). The in-app ring buffer (`GET /api/admin/audit`)
